...
# 以POST方式传输检测结果
def post_msg(server_url, up, down):
    # img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    up_str = cv2.imencode('.jpg', up)[1].tostring()
    up_bin = base64.b64encode(up_str)
    down_str = cv2.imencode('.jpg', down)[1].tostring()
...
def run():
    ...
    while True:
        ...
        # 以POST方式传输处理后的整张图片
        try:
            post_msg(skill_cfg['server_url'], up_resized, down_resized)
